...
silent_spring_text <- pdf_text(here("data", "silent_spring.pdf"))
...
```{r}
ddt_background <- readJPEG(here("ddt_spraying.jpg"))
```
...
ddt_for_me <- readJPEG(here("ddt1.jpg"))
```
...
ddt_plane <- readJPEG(here("ddt_plane.jpg"))
...
```{r,  message = FALSE}
silent_spring_bing <- silent_spring_nonstop_words %>% 
  inner_join(get_sentiments("bing")) 

silent_spring_bing_counts <- silent_spring_bing %>% 
  count(sentiment)

ddt_spraying <- readJPEG(here("DDT-spraying.jpg"))

ggplot(data = silent_spring_bing_counts, aes(x = sentiment, y = n)) +
  annotation_custom(rasterGrob(ddt_spraying, 
                               width = unit(1,"npc"), 
                               height = unit(1,"npc")), 
                    -Inf, Inf, -Inf, Inf) +
  geom_col() +
  labs(title = "Silent Spring Bing Sentiment Analysis",
       x = "Sentiment",
       y = "Word Count")
```
...
silent_spring_afinn <- silent_spring_nonstop_words %>% 
  inner_join(get_sentiments("afinn")) 

silent_spring_afinn_counts <- silent_spring_afinn %>% 
  count(sentiment)


ggplot(data = silent_spring_afinn_counts, aes(x = sentiment, y = n)) +
  geom_col()
